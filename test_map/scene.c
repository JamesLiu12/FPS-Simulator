...
#include "../obj_converter/out/clock.c"
#include "map.h"
...
void Scene_Init(struct Scene *scene){
    ...
    struct Mesh *mesh0 = ModelClock_New();
    ModelClock_Init(mesh0);
    Transform_Init(&transform, NULL);

    struct Object* objectFloor = Object_New(mesh0, &transform, FLOOR, collideBoxes, collideBoxCount);
    ...
    struct Mesh *mesh1 = ModelTest_boundary_New();
    Transform_Init(&transform_Boundary, NULL);

    struct Object* Map_Boundary = Object_New(mesh1, &transform_Boundary, WALL, collideBoxes_Boundary, collideBoxCount_Boundary);
    ...
    struct Mesh *mesh2 = ModelWall_New();
    ModelWall_Init(mesh2);
    Transform_Init(&transform_Barrier, NULL);

    struct Object* Map_Barrier = Object_New(mesh2, &transform_Barrier, FLOOR, collideBoxes_Barrier, collideBoxCount_Barrier);
